...
file = open('charlie_results/list_ids.csv', 'w')
for item in list_ids:
    file.write("%s\n" % item)

# load model that contains all words from media cloud articles database
modelMC = Word2Vec.load('data/MediaCloud_w2v')

# docvs is the document matrix, where each row is a document
# and the columns are the atributtes from word2vec
...
nx.write_gpickle(G, 'charlie_results/empirical_graph.gpickle')

H = create_graphml(dists_triu, articles)
nx.write_graphml(H, 'charlie_results/empirical_graph.graphml')

empirical_nodes = G.nodes()
np.savetxt('charlie_results/empirical_graph_nodes.csv', empirical_nodes, delimiter=',')

all_nodes_domains = []
for i in empirical_nodes:
    all_nodes_domains.append(G.node[i]['domain'])

f = open('charlie_results/graph_original_domains_each_node.txt', 'w')
...
np.savetxt('charlie_results/graph_complete.csv', as_numpy, delimiter=',')
...
np.savetxt('charlie_results/i0.csv', I0, delimiter=',')
