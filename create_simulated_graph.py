...
Infects = np.loadtxt(open('charlie_results/Infects.csv'), delimiter=",")
G = np.loadtxt(open('charlie_results/graph_complete.csv'), delimiter=",")
original_graph = nx.read_gpickle('charlie_results/empirical_graph.gpickle')
#
ori_outs = [i[1] for i in original_graph.out_degree_iter()]
g_ori = gaussian_kde(ori_outs)
#
graph_simulated = graph_simu_nx(Infects, G)
nx.write_gpickle(graph_simulated, 'charlie_results/graph_simulated_nx.gpickle')
...
plt.xlim(0, 20)
plt.savefig('charlie_results/graph_validation.png')
plt.show()
